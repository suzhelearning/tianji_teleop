...
class RecoveryTests(unittest.TestCase):
    def test_native_status_partial_lines_visible_without_blocking(self):
        ...
        with patch('sim.mapped_recovery.os.read',side_effect=[b'MAPPED: S rej',BlockingIOError()]):
            self.update(1_000_000_000)
        self.assertNotIn('rejected',self.control.status)
        with patch('sim.mapped_recovery.os.read',side_effect=[b'ected: stale PICO input\n',BlockingIOError()]):
    ...
    def update(self, now):
        with patch('sim.mapped_recovery.time.monotonic_ns',return_value=now),patch('sim.mapped_recovery.os.write',side_effect=lambda fd,data:self.sent.append(data) or len(data)):
            self.control.update()
