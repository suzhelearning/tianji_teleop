...
BUILD = ROOT / 'control/build-mapped-palm'
sys.path.insert(0, str(ROOT))
from real_robot.protocol import decode_packet, ARMS_READY
from sim.mapped_overlay import decode as decode_target
...
class PortTests(unittest.TestCase):
    def test_recovery_requires_fresh_rest_then_explicit_start(self):
        with socket.socket(socket.AF_INET,socket.SOCK_DGRAM) as output, socket.socket(socket.AF_INET,socket.SOCK_DGRAM) as reserve:
            ...
            command=[str(BUILD/'mapped_palm_tjrc_controller'),'--config',str(BUNDLE/'config/deployment.yaml'),
    ...
    def test_xz_calibration_requires_c_then_s_and_stable_input(self):
        with socket.socket(socket.AF_INET,socket.SOCK_DGRAM) as output, socket.socket(socket.AF_INET,socket.SOCK_DGRAM) as reserve:
            ...
            args=[str(BUILD/'mapped_palm_tjrc_controller'), '--config', str(BUNDLE/'config/deployment.yaml'),
                  '--model', str(BUNDLE/'assets/mapped_palm/marvin_m6_wuji2.xml'),
                  '--joint-command-host','127.0.0.1','--joint-command-port',str(output.getsockname()[1]),
    ...
    def test_same_input_matches_validated_source_worker(self):
        ...
        actual=worker(BUNDLE,BUILD/'mapped_palm_native_worker',requests)
