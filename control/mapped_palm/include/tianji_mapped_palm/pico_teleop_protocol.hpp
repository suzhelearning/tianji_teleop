...
struct PicoTeleopFrame {
  // Receiver-local diagnostics only; no TJVR wire/schema change. Last accepted
  // epoch/discontinuity event is retained across latest-only frame supersession.
  struct ResetDiagnostic {
    std::uint64_t sequence{0}, previous_sequence{0}, previous_epoch{0};
    std::int64_t receive_ns{0}, accepted_gap_ns{0};
    double left_position_m{0}, right_position_m{0};
    double left_orientation_rad{0}, right_orientation_rad{0};
  } reset_diagnostic;
