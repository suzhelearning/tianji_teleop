...
VelocityQpSmoothnessCosts evaluateVelocityQpSmoothnessCosts(
    const HierarchicalQpConfig& config, const ArmIkInput& input,
    const Vec7& qdot) noexcept {
  VelocityQpSmoothnessCosts result;
  if (config.smoothness_mode != VelocityQpSmoothnessMode::kNormalizedSplit ||
      !std::isfinite(input.dt) || input.dt <= 0.0 || !qdot.allFinite() ||
      !input.qdot_prev.allFinite() || !input.qddot_prev.allFinite()) {
    return result;
  }
  const Vec7 acceleration = (qdot - input.qdot_prev) / input.dt;
  const Vec7 jerk = (acceleration - input.qddot_prev) / input.dt;
  result.full_acceleration = config.full_acceleration_weight *
      (acceleration.array() /
       config.full_acceleration_scale_rad_s2.array()).matrix().squaredNorm();
  result.full_jerk = config.full_jerk_weight *
      (jerk.array() /
       config.full_jerk_scale_rad_s3.array()).matrix().squaredNorm();
  if (input.vector_nullspace_task.active &&
      input.vector_nullspace_task.basis.allFinite() &&
      input.vector_nullspace_task.basis.norm() > 1.0e-12) {
    const Vec7 basis = input.vector_nullspace_task.basis.normalized();
    result.nullspace_acceleration = config.nullspace_acceleration_weight *
        std::pow(basis.dot(acceleration) /
                     config.nullspace_acceleration_scale_rad_s2,
                 2);
    result.nullspace_jerk = config.nullspace_jerk_weight *
        std::pow(basis.dot(jerk) / config.nullspace_jerk_scale_rad_s3, 2);
  }
  return result;
}
...
HierarchicalQpProblem HierarchicalQpBuilder::build(const ArmIkInput& input) const {
  ...
  const bool normalized_smoothness =
      config_.smoothness_mode == VelocityQpSmoothnessMode::kNormalizedSplit;
  const double base_qdot_weight =
      config_.lambda_reg +
      (normalized_smoothness ? 0.0 : config_.continuity_weight) +
      (normalized_smoothness ? 0.0 : config_.jerk_weight);
  ...
  problem.g.head<kArmDof>().setZero();
  if (!normalized_smoothness) {
    problem.g.head<kArmDof>() =
        -config_.continuity_weight * input.qdot_prev -
        config_.jerk_weight *
            (input.qdot_prev + input.qddot_prev * input.dt);
  } else if (std::isfinite(input.dt) && input.dt > 0.0) {
    const Vec7 acceleration_target = input.qdot_prev;
    const Vec7 jerk_target = input.qdot_prev + input.qddot_prev * input.dt;
    for (int joint = 0; joint < kArmDof; ++joint) {
      const double acceleration_denominator =
          config_.full_acceleration_scale_rad_s2[joint] * input.dt;
      const double jerk_denominator =
          config_.full_jerk_scale_rad_s3[joint] * input.dt * input.dt;
      const double acceleration_curvature =
          config_.full_acceleration_weight /
          (acceleration_denominator * acceleration_denominator);
      const double jerk_curvature =
          config_.full_jerk_weight /
          (jerk_denominator * jerk_denominator);
      problem.H(joint, joint) +=
          acceleration_curvature + jerk_curvature;
      problem.g[joint] -=
          acceleration_curvature * acceleration_target[joint] +
          jerk_curvature * jerk_target[joint];
    }
  }
  for (int joint = 0; joint < kArmDof; ++joint) {
  ...
  problem.vector_nullspace_task = input.vector_nullspace_task;
  if (input.vector_nullspace_task.active &&
      input.vector_nullspace_task.basis.allFinite() &&
      input.vector_nullspace_task.basis.norm() > 1.0e-12 &&
      std::isfinite(input.vector_nullspace_task.alpha_command) &&
      std::isfinite(input.vector_nullspace_task.alpha_previous) &&
      std::isfinite(input.vector_nullspace_task.alpha_trend) &&
      std::isfinite(input.vector_nullspace_task.reference_weight) &&
      input.vector_nullspace_task.reference_weight >= 0.0 &&
      std::isfinite(input.vector_nullspace_task.continuity_weight) &&
      input.vector_nullspace_task.continuity_weight >= 0.0 &&
      std::isfinite(input.vector_nullspace_task.jerk_weight) &&
      input.vector_nullspace_task.jerk_weight >= 0.0) {
    const Vec7 basis = input.vector_nullspace_task.basis.normalized();
    double acceleration_curvature =
        input.vector_nullspace_task.continuity_weight;
    double jerk_curvature = input.vector_nullspace_task.jerk_weight;
    double acceleration_target = input.vector_nullspace_task.alpha_previous;
    double jerk_target = input.vector_nullspace_task.alpha_trend;
    if (normalized_smoothness && std::isfinite(input.dt) && input.dt > 0.0) {
      const double acceleration_denominator =
          config_.nullspace_acceleration_scale_rad_s2 * input.dt;
      const double jerk_denominator =
          config_.nullspace_jerk_scale_rad_s3 * input.dt * input.dt;
      acceleration_curvature = config_.nullspace_acceleration_weight /
          (acceleration_denominator * acceleration_denominator);
      jerk_curvature = config_.nullspace_jerk_weight /
          (jerk_denominator * jerk_denominator);
      acceleration_target = basis.dot(input.qdot_prev);
      jerk_target = basis.dot(
          input.qdot_prev + input.qddot_prev * input.dt);
    }
    ...
    problem.g.head<kArmDof>().noalias() -=
        (input.vector_nullspace_task.reference_weight *
             input.vector_nullspace_task.alpha_command +
         acceleration_curvature * acceleration_target +
         jerk_curvature * jerk_target) *
        basis;
  }
...
SafetyDecision validateHierarchicalProblem(
  ...
  if (problem.vector_nullspace_task.active &&
      (!problem.vector_nullspace_task.basis.allFinite() ||
       problem.vector_nullspace_task.basis.norm() <= 1.0e-12 ||
       !std::isfinite(problem.vector_nullspace_task.alpha_command) ||
       !std::isfinite(problem.vector_nullspace_task.alpha_previous) ||
       !std::isfinite(problem.vector_nullspace_task.alpha_trend) ||
       !std::isfinite(problem.vector_nullspace_task.reference_weight) ||
       problem.vector_nullspace_task.reference_weight < 0.0 ||
       !std::isfinite(problem.vector_nullspace_task.continuity_weight) ||
       problem.vector_nullspace_task.continuity_weight < 0.0 ||
       !std::isfinite(problem.vector_nullspace_task.jerk_weight) ||
       problem.vector_nullspace_task.jerk_weight < 0.0)) {
    return {false, HoldReason::kNonFiniteProblem, -1};
  }
