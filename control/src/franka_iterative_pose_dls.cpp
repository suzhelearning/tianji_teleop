...
FrankaIterativePoseDlsIk7::FrankaIterativePoseDlsIk7(IterativeDlsConfig config,
                                         double margin_rad)
    : config_(std::move(config)), margin_rad_(margin_rad) {}

PoseDlsResult FrankaIterativePoseDlsIk7::solve(const FrankaPoseDlsInput& input) const {
  ...
  const double initial_merit = merit(error);
  double best_merit = initial_merit;
  ...
  for (int iteration = 0; iteration < config_.max_iterations; ++iteration) {
    ...
    const Vec7 nominal = input.fixed_posture_reference_valid &&
                                 input.fixed_posture_reference.allFinite()
                             ? frankaClampPostureReferenceToInterior(
                                   input.fixed_posture_reference, input.limits,
                                   margin_rad_)
                             : 0.5 * (input.limits.lower_position +
                                      input.limits.upper_position);
    ...
    if (input.constrain_step) {
      step = input.constrain_step(current, sample, damping, step);
      // A common scale preserves the corrected direction and joint-step cap.
      const double scale = std::max(
          {1.0, step.cwiseAbs().maxCoeff() / config_.maximum_joint_step_rad,
           step.norm() / config_.maximum_step_norm_rad});
      step /= scale;
    }
  ...
  if (input.return_final_iterate) {
    // current/sample/error advance only after a finite, feasible candidate.
    // Nonconvergence is diagnostic, not a reason to discard the final iterate.
    result.q = current;
    result.position_error_m = error.head<3>().norm();
    result.orientation_error_rad = error.tail<3>().norm();
    if (converged(error)) {
      ...
      result.status =
          merit(error) + config_.minimum_merit_improvement <
                  initial_merit
              ? PoseDlsStatus::kImproved
              : PoseDlsStatus::kNotConverged;
      result.detail = "pose_dls_final_iterate_not_converged";
    }
    result.solve_time_us = elapsedMicroseconds(start);
    return result;
  }
  ...
  if (converged(best_error)) {
    result.status = PoseDlsStatus::kConverged;
  } else if (best.allFinite() && best_error.allFinite()) {
    // The seed is a valid candidate too.  A bounded solve that cannot improve
    // it is still a usable best-effort IK result, not a rejected solve that
    // should make the caller restore a previous target.
    result.status = best_updated ? PoseDlsStatus::kImproved
                                 : PoseDlsStatus::kNotConverged;
    result.detail = best_updated ? "pose_dls_iteration_limited"
                                 : "pose_dls_no_improvement";
  }
  result.solve_time_us = elapsedMicroseconds(start);
  return result;
}
