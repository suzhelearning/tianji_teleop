...
SharedRootOptions loadSharedRootOptions(const std::string& path) {
  ...
  require(out.input_sha256=="68304d5b76b063996bdf386b28d103af1d998f7f9a801acb5a55960aaa2fccda","unsupported input contract revision");
  ...
  require(out.geometry_sha256=="41a747a39a7f8ba09311b3a143292296b6d72c1d5999dea6de5599bcb611b937" ||
      out.geometry_sha256=="6404caae29c0317e42abf6b3641e66c9ce189772905161231f59bd4ef036c690",
  ...
  const auto repository=input.parent_path().parent_path().parent_path();
  for(const auto& file:ic["source_files"])
    fingerprint(repository/file.first.as<std::string>(),file.second.as<std::string>());
  out.urdf_path=fs::canonical(geometry.parent_path()/g["urdf_path"].as<std::string>()).string();
  out.mujoco_path=fs::canonical(geometry.parent_path()/g["mujoco_xml_path"].as<std::string>()).string();
