...
DEFAULT_HOME_CONFIG = Path(__file__).resolve().parent / "mapped_palm/config/home.yaml"
...
def load_home(path=DEFAULT_HOME_CONFIG):
...
def load_controller_posture(path):
    ...
    if any(key in controller for key in keys):
        ...
        posture = load_home(path.parent / home_path)
    elif enabled:
