...
constexpr auto profile = TIANJI_PROJECT_SOURCE_DIR "/config/qp_ik_pico_shared_root_ceres.yaml";
class CeresController : public ::testing::Test {
 protected:
  ...
  CeresController() {
    config.pico_ee_franka_ceres_lm.max_solver_time_seconds = .1;
...
TEST_F(CeresController, ProfileIsOptInAndDefaultUnchanged) {
  EXPECT_TRUE(config.shared_root_profile_path.empty());
  EXPECT_EQ(config.ik_algorithm, IkAlgorithm::kPicoEeFrankaCeresLm);
...
TEST_F(CeresController, SimulationSoftStartUsesLowDynamicLimits) {
  DualArmController controller(robot,config);
  ASSERT_TRUE(controller.beginSimulationSoftStart());
  auto t=target();t.left.position.x()+=.05;t.right.position.x()+=.05;
  for(int n=0;n<30;++n) {
    const auto previous=controller.referenceState(ArmSide::kRight);
    auto result=controller.step(t,.005);
    ASSERT_TRUE(result.accepted)<<result.right.dls_posture_ruckig_detail;
    const auto state=controller.referenceState(ArmSide::kRight);
    EXPECT_LE(state.qdot.cwiseAbs().maxCoeff(),.35+1e-8);
    EXPECT_LE(state.qddot.cwiseAbs().maxCoeff(),.5+1e-8);
    EXPECT_LE((state.qddot-previous.qddot).cwiseAbs().maxCoeff()/.005,2.+1e-6);
  }
}
TEST_F(CeresController, ExplicitPinocchioIsRequiredWithoutFallback) {
...
TEST_F(CeresController, PinocchioTcpAndJacobianMatchBothModelAndFullPinocchio) {
...
TEST_F(CeresController, ReportsPinocchioAndSeparatePipelineTiming) {
  DualArmController controller(robot,config);
...
TEST_F(CeresController, SourceModelAndRuckigLimits) {
  ...
  const auto& post=config.pico_ee_franka_ceres_lm.post_smoothing;
...
TEST_F(CeresController, KinematicsOnlyMatchesFullModelAndDoesNotMutateLiveState) {
  for(auto side:{ArmSide::kLeft,ArmSide::kRight}) {
...
TEST_F(CeresController, RejectsHardwareDisabledAndRelaxedEnvelope) {
  auto c=config; c.controller.model_state_only=false;
  EXPECT_THROW(DualArmController(robot,c),std::invalid_argument);
  c=config; c.pico_ee_franka_ceres_lm.enabled=false;
  EXPECT_THROW(DualArmController(robot,c),std::invalid_argument);
  c=config; c.pico_ee_franka_ceres_lm.post_smoothing.max_jerk_rad_s3*=2;
  EXPECT_THROW(DualArmController(robot,c),std::invalid_argument);
  c=config; c.pico_ee_franka_ceres_lm.post_smoothing.max_velocity_rad_s[0]=std::numeric_limits<double>::quiet_NaN();
  EXPECT_THROW(DualArmController(robot,c),std::invalid_argument);
}
TEST_F(CeresController, StationaryAndMovingPoseRemainBounded) {
  DualArmController controller(robot,config);
  auto t=target();
  ...
  for(int n=0;n<150;++n) {
    ...
    EXPECT_TRUE((state.qdot.cwiseAbs().array()<=config.pico_ee_franka_ceres_lm.post_smoothing.max_velocity_rad_s.array()+1e-8).all());
...
TEST_F(CeresController, StaleDoesNotChaseLastGoalAndRecovers) {
  DualArmController controller(robot,config);
...
TEST_F(CeresController, BadDtCannotMoveEitherArm) {
  DualArmController controller(robot,config);
...
TEST_F(CeresController, OneInvalidResetCannotPartiallyCommitOtherArm) {
  DualArmController controller(robot,config);
...
TEST_F(CeresController, ResetDropsOldGoalSeed) {
  DualArmController controller(robot,config);
