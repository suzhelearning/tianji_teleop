...
TEST(CeresTrajectorySoftStart, SlowApproachThenRampWithoutStateReset) {
  ...
  CeresTrajectoryLimiter7 limiter(cfg,bounds,.005);
...
TEST(CeresTrajectoryLimiter, GeneratesBoundedContinuousStepResponse) {
  constexpr double kDt = 0.005;
  CeresTrajectoryLimiter7 limiter(config(), limits(), kDt);
  ...
  for (int sample = 0; sample < 1000; ++sample) {
    const CeresTrajectoryResult result = limiter.update(target, kDt);
    ASSERT_TRUE(result.accepted) << result.detail;
...
TEST(CeresTrajectoryLimiter, ResetSynchronizesAllReferenceDerivatives) {
  CeresTrajectoryLimiter7 limiter(config(), limits(), 0.005);
  ArmMotionState initial;
...
TEST(CeresTrajectoryLimiter, ProjectsDiscreteVelocityOvershootAtTheLimit) {
  constexpr double kDt = 0.005;
  CeresTrajectoryLimiter7 limiter(fastConfig(), fastLimits(), kDt);
  ArmMotionState initial;
  initial.qdot = Vec7::Constant(3.9);
  initial.qddot = Vec7::Constant(30.0);
  ASSERT_TRUE(limiter.reset(initial));

  const CeresTrajectoryResult result = limiter.update(Vec7::Constant(1.0), kDt);
  ASSERT_TRUE(result.accepted) << result.detail;
...
TEST(CeresTrajectoryLimiter, ClipsOutwardSamplesAtBothPositionLimits) {
  constexpr double kDt = 0.005;
  ...
  for (const double direction : {-1.0, 1.0}) {
    CeresTrajectoryLimiter7 limiter(config(), safe, kDt);
    ...
    ASSERT_TRUE(result.accepted) << result.detail;
    EXPECT_EQ(result.detail, "joint_trajectory_position_projected");
...
TEST(CeresTrajectoryLimiter, SettledRoundoffDoesNotRejectOrPreventRestart) {
  constexpr double kDt = 0.005;
  auto cfg = fastConfig();
  cfg.max_acceleration_rad_s2.tail<4>().setConstant(90.0);
  cfg.max_jerk_rad_s3.tail<4>().setConstant(4500.0);
  CeresTrajectoryLimiter7 limiter(cfg, fastLimits(), kDt);
...
TEST(CeresTrajectoryLimiter, AtTargetWithRealVelocityStillBrakes) {
  CeresTrajectoryLimiter7 limiter(config(), limits(), 0.005);
  ArmMotionState initial;
...
TEST(CeresTrajectoryLimiter, SettledAccumulatedRoundoffKeepsJerkCheck) {
  CeresTrajectoryLimiter7 limiter(fastConfig(), fastLimits(), 0.005);
  ArmMotionState initial;
