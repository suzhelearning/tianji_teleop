...
class PolynomialCamera {
 ...
 public:
  ...
  PolynomialCamera(double width, double height, double fx, double fy, double cx, double cy,
                   double skew, double k2 = 0.0, double k3 = 0.0, double k4 = 0.0, double k5 = 0.0,
                   double k6 = 0.0, double k7 = 0.0)
      : fx_(fx), fy_(fy), cx_(cx), cy_(cy), skew_(skew), distortion_(std::abs(k2) > 1e-7) {
    k2_ = k2;
    k3_ = k3;
    k4_ = k4;
    k5_ = k5;
    k6_ = k6;
    k7_ = k7;
    std::cout << "[PolynomialCamera] distortion enabled: " << (distortion_ ? "YES" : "NO")
              << ", k2=" << k2_ << std::endl;
  }
  ...
  Vector3d cam2world(const double& u, const double& v) const {
    ...
    if (!distortion_) {
      ...
      for (int i = 0; i < 7; ++i) {
        ...
        theta = thetad / (1.0 + k2_ * theta + k3_ * theta2 + k4_ * theta3 + k5_ * theta4 +
                          k6_ * theta5 + k7_ * theta6);
      }
  ...
  Vector3d cam2world(const Vector2d& px) const { return cam2world(px[0], px[1]); }
  ...
  Vector2d world2cam(const Vector2d& uv) const {
    ...
    if (!distortion_) {
      ...
      if (r < 1e-8) {
        // At optical center, no distortion needed
        px[0] = cx_;
        px[1] = cy_;
        return px;
      }
  ...
  inline double thetad_from_theta(const double theta) const {
    ...
    const double thetad = theta + k2_ * theta2 + k3_ * theta3 + k4_ * theta4 + k5_ * theta5 +
