...
def generate_launch_description():
    ...
    enable_local_pose_arg = DeclareLaunchArgument(
        'enable_local_pose',
        default_value='true',
        description='Publish odin1 odometry transformed into the map frame after relocalization'
    )
    ...
    enable_pcl_tools_arg = DeclareLaunchArgument(
        'enable_pcl_tools',
        default_value='true',
        description='Launch optional PCL/depth/reprojection/overlay post-processing nodes'
    )
    ...
    pcd2depth_node = Node(
        ...
        output='screen',
        condition=IfCondition(LaunchConfiguration('enable_pcl_tools')),
    ...
    local_pose_node = Node(
        package='odin_ros_driver',
        executable='local_pose_node',
        name='local_pose_node',
        output='screen',
        condition=IfCondition(LaunchConfiguration('enable_local_pose')),
        parameters=[{
            'odometry_topic': '/odin1/odometry',
            'target_frame': 'map',
            'local_pose_topic': '/odin1/local_pose',
            'local_odometry_topic': '/odin1/local_odometry',
        }]
    )
    ...
    ld.add_action(enable_local_pose_arg)
    ld.add_action(enable_rviz_arg)
    ld.add_action(enable_pcl_tools_arg)
