...
READY = runpy.run_path(str(ROOT / "tracking/scripts/check_pico_session_ready.py"))
...
def test_skeleton_heartbeat_is_only_sent_after_viewer_sync(monkeypatch, sync_fails):
    ...
    module = runpy.run_path(str(ROOT / "tracking/src/pico_bridge/scripts/smpl_mujoco_visualizer.py"))
    events = []
...
def test_conflict_inspection_never_signals(tmp_path, monkeypatch):
    cleaner = runpy.run_path(str(ROOT / "tracking/scripts/cleanup_tianji_pico_processes.py"))
...
@pytest.mark.parametrize("mode,expected", [("--sim", "project"), ("--real", "real"), ("--data", "real")])
def test_real_python_override_is_mode_specific(tmp_path, mode, expected):
    import shutil
    shutil.copy(ROOT / "teleop.sh", tmp_path)
    (tmp_path / "scripts").mkdir()
    shutil.copy(ROOT / "scripts/environment.sh", tmp_path / "scripts")
    for name in ("project", "real"):
        script = tmp_path / name
        script.write_text("#!/bin/sh\nprintf '%s\\n' " + name + "\n")
        script.chmod(0o755)
    result = subprocess.run(["bash", str(tmp_path / "teleop.sh"), mode, "--help"],
        env={**os.environ, "TIANJI_PYTHON": str(tmp_path / "project"),
             "TIANJI_REAL_PYTHON": str(tmp_path / "real")}, capture_output=True, text=True, timeout=5)
...
def test_launcher_propagates_readiness_failure_without_cleanup(tmp_path, ready_rc):
    ...
    (tmp_path / "install/local_setup.bash").touch()
    shutil.copy(ROOT / "tracking/scripts/start_tianji_pico_teleop.sh", scripts)
