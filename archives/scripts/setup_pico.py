...
sys.path.insert(0, str(ROOT / "tracking/scripts"))
from scripts.calibrate_pico_simple import calibrate
...
def preflight(root):
    ...
    if not (root / "tracking/install/local_setup.bash").is_file():
        raise RuntimeError("请先运行 pixi run -e tracking build-tracking")
...
def setup(user, height, timeout=30, root=ROOT, *, check=preflight,
    ...
    with (directory / "driver.log").open("x") as log:
        driver = popen(["bash", str(root / "tracking/scripts/start_pico_driver.sh")],
    ...
    result = run(["bash", str(root / "tracking/scripts/start_tianji_pico_teleop.sh"),
    ...
    print("请检查骨架、掌心位置和转腕方向。确认后另开终端运行 pixi run sim。\n"
          "输入会话继续运行；停止：pixi run -e tracking stop-pico", flush=True)
    return 0
