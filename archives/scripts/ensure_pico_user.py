...
from scripts.calibrate_pico_simple import resolve
...
def ensure(user, root=ROOT, run=subprocess.run, resolver=resolve):
    ...
    if present.returncode:
        ...
        run(["bash", str(root / "tracking/scripts/start_tianji_pico_teleop.sh"), "--detach",
    ...
    if (option("@tianji_checkout") != str(root / "tracking") or
    ...
    run([sys.executable, str(root / "tracking/scripts/check_pico_session_ready.py"),
         "--session", SESSION, "--checkout", str(root / "tracking"), "--timeout-s", "30"], check=True)
