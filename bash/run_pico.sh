...
case "${1:-}" in
  --help|-h) usage; exit 0 ;;
  --list-users) exec "$TIANJI_PYTHON" "$root/src/teleop_inputs/pico_bridge/scripts/calibrate_pico_simple.py" --list-users ;;
  --user) [[ $# -eq 2 && -n "$2" ]] || { usage >&2; exit 2; } ;;
  *) usage >&2; exit 2 ;;
esac
exec "$TIANJI_PYTHON" "$root/src/teleop_inputs/pico_bridge/scripts/ensure_pico_user.py" "$@"
