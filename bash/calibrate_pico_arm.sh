...
默认 ROS_DOMAIN_ID=$ROS_DOMAIN_ID，ROS_AUTOMATIC_DISCOVERY_RANGE=$ROS_AUTOMATIC_DISCOVERY_RANGE
EOF
}
...
if [[ -n "$calibration_user" ]]; then
  if [[ "${1:-}" == status ]]; then
    if ! PICO_CALIBRATION_DIR="$(env -u PICO_CALIBRATION_DIR "$profile_python" \
        -m teleop_profile --user "$calibration_user" --component pico)"; then
    ...
    exec "$profile_python" -m teleop_profile \
      --user "$calibration_user" --calibrate -- \
      bash "$repo_root/bash/calibrate_pico_arm.sh" "$@"
  fi
fi
...
validate_artifact() {
  ...
  local side="$3"
  local summary
  summary="$("$calibration_python" "$pico_scripts/pico_calibration_artifact.py" validate \
    --path "$path" \
    --kind "$kind" \
    --side "$side" \
...
activate_calibration_candidate() {
  ...
  local summary
  summary="$("$calibration_python" "$pico_scripts/pico_calibration_artifact.py" activate \
    --path "$candidate" \
    --active "$active" \
    --kind "$kind" \
    --side "$side" \
...
run_geometry() {
  ...
  geometry_arguments+=("$@")
  if ! bash "$pico_scripts/pico_arm_geometry_runtime.sh" \
  ...
  if [[ "$geometry_policy" == symmetric_max ]]; then
    if ! "$profile_python" "$pico_scripts/pico_symmetric_profile.py" --source "$calibration_dir"; then
...
interactive_menu() {
  ...
  while true; do
    echo
    echo "PICO 标定菜单 (ROS_DOMAIN_ID=$ROS_DOMAIN_ID, discovery=$ROS_AUTOMATIC_DISCOVERY_RANGE)"
    echo "  1) 单项标定"
...
source "$repo_root/bash/environment.sh"
source "$pico_scripts/pico_process_cleanup.sh"
