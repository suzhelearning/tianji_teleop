...
from tianji_runtime.resources import workspace as _workspace


# Profiles live at the workspace root, not beside this module.
ROOT = _workspace()
...
def resolve_profile(user: str, component: str, root: Path = ROOT) -> str:
    ...
    if component == "manus":
        calibration_path = root / "src" / "teleop_inputs" / "manus_bridge" / "calibration"
...
def _validate_pico_revision(revision: Path) -> None:
    ...
    scripts = Path(get_package_prefix("pico_bridge")) / "lib" / "pico_bridge"
