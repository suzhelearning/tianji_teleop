...
from data_collector import dataset  # noqa: E402
from data_collector.dataset import (  # noqa: E402
    DATASET_CONFIG_NAME,
...
def test_partial_then_complete_roundtrip(tmp_path: Path) -> None:
    ...
    written = json.loads((writer.partial_path.parent / DATASET_CONFIG_NAME).read_text('utf-8'))
...
def test_date_configs_can_differ_when_reservation_crosses_midnight(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
    now = datetime(2026, 9, 13, 23, 59, 59)
    next_day = datetime(2026, 9, 14, 0, 0, 0)
    monkeypatch.setattr(dataset.time, 'time_ns', lambda: int(now.timestamp() * 1e9))
    reserve = EpisodeWriter._reserve_episode

    def reserve_before_midnight(self):
        nonlocal now
        result = reserve(self)
        now = next_day
        return result

    monkeypatch.setattr(EpisodeWriter, '_reserve_episode', reserve_before_midnight)
    first_config = make_config()
    second_config = make_config(image_encoding='rgb', jpeg_quality=None, camera_names=['top'])
    first = EpisodeWriter(tmp_path, first_config, TASK)
    first.abort()
    second = EpisodeWriter(tmp_path, second_config, TASK)
    second.abort()

    assert first.partial_path.parent == tmp_path / '20260913'
    assert second.partial_path.parent == tmp_path / '20260914'
    assert load_dataset_config(first.partial_path.parent) == normalize_dataset_config(first_config)
