...
```bash
# 在 Pixi 环境中运行，先 source 工作空间 overlay（install/<env>/local_setup.bash）

# === Wuji Hand（默认手型）===
# 不带 --config 时，--wuji-glove 默认加载 config/adaptive_analytical_wuji_glove_<hand>.yaml
python -m example.tuning_tool --wuji-glove --hand right --glove-sn <YOUR_SN>
python -m example.tuning_tool --wuji-glove --hand left  --glove-sn <YOUR_SN>
