...
- Internalized hand retargeting, calibration commands, and populated model/simulator assets as the `wuji_retargeting` ament package under `src/wuji/wuji_retargeting/`; Python imports now use `wuji_retargeting` and the top-level `example` package, and colcon builds the pybind11 `_native` kernel from `wuji_retargeting/native.cpp`. Upstream licenses remain with the assets; no runtime submodule initialization is required.
- Manus launch runs in the Pixi-managed environment and retains sourced ROS 2 paths, while preserving paired calibration files and the simulation-only TJH2 safety boundary.
