...
# This file lives at <prefix>/wuji_retargeting/viz/tuning_viewer.py: the package
# directory holds the bundled hand models and its sibling `example` package
# holds the default retarget configs.
_PACKAGE_DIR = Path(__file__).resolve().parent.parent
_EXAMPLE_CONFIG_DIR = _PACKAGE_DIR.parent / "example" / "config"
    ...
    def __init__(
        ...
        if retarget_config_path is None:
            retarget_config_path = str(
                _EXAMPLE_CONFIG_DIR / "adaptive_analytical_avp.yaml"
            )
        ...
        # to the bundled hand for this side under the owned wuji-description assets.
        if mjcf_path is not None:
            mjcf_path = Path(mjcf_path)
        else:
            mjcf_path = (
                _PACKAGE_DIR / "wuji-description" / "hand" / "body"
