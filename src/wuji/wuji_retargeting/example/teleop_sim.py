...
from example.utils.config_paths import resolve_mjcf_path, qpos_reorder_perm
try:
...
# This file is <prefix>/example/teleop_sim.py, so its own directory ships the
# example configs and the sibling `wuji_retargeting` package ships the models.
_EXAMPLE_DIR = Path(__file__).resolve().parent
_WUJI_RETARGETING_DIR = _EXAMPLE_DIR.parent / "wuji_retargeting"
...
def run_tuning_mode(
    ...
    from example import tuning_tool
...
def run_teleop(
    ...
    if mjcf_override:
        ...
        mjcf_path = (
            _WUJI_RETARGETING_DIR
            / "wuji-description" / "hand" / "body" / "mjcf" / f"{hand_side}.xml"
        )
