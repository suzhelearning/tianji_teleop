...
from tianji_controller.mapped_calibration import CalibrationGuard
from tianji_controller.mapped_events import EventFrame
...
class CalibrationTests(unittest.TestCase):
    def test_real_entry_c_then_enter_lock_precedes_mock_enable(self):
        from tianji_controller import run_teleop
        events=[]
        device=MagicMock()
        ...
        with patch('tianji_controller.run_teleop.sys.stdin.isatty',return_value=True), \
             patch('tianji_controller.run_teleop.make_hardware',return_value={'arms':device}), \
             patch('tianji_controller.run_teleop.RealRobotViewer'), \
             patch('tianji_controller.run_teleop.CommandReceiver'), \
             patch('tianji_controller.mapped_events.EventReceiver',Receiver), \
             patch('tianji_runtime.OperatorKeyboard',Keyboard), \
             patch('tianji_controller.run_teleop.ExecutorObserver'), \
             patch('tianji_controller.run_teleop.FeedbackHub'), \
             patch('tianji_controller.run_teleop.subprocess.Popen',return_value=child), \
             patch('tianji_controller.run_teleop.stop_controller'), \
    ...
    def test_c_never_arms_and_enter_requires_lock(self):
        ...
        with patch('tianji_controller.mapped_calibration.time.monotonic_ns',return_value=NOW):
    ...
    def test_optional_keyboard_callback_keeps_enter(self):
        ...
        calls=[]
        with patch('tianji_runtime.keyboard.sys.stdin',SimpleNamespace(fileno=lambda:123)), \
             patch('tianji_runtime.keyboard.termios.tcgetattr',return_value=[]), \
             patch('tianji_runtime.keyboard.termios.tcsetattr'), \
             patch('tianji_runtime.keyboard.tty.setcbreak'), \
             patch('tianji_runtime.keyboard.select.select',return_value=([123],[],[])), \
             patch('tianji_runtime.keyboard.os.read',side_effect=[b'C',b'\n']):
            keyboard=OperatorKeyboard(calls.append,on_calibrate=lambda:calls.append('calibrate'))
