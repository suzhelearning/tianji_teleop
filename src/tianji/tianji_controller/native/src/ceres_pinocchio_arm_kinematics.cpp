...
struct CeresPinocchioArmKinematics::Impl {
  struct ArmMapping {
  ...
  ArmMapping buildMapping(ArmSide side, const Pose& tcp) {
    ...
    if (!tcp.position.allFinite() || !isProperRotation(tcp.rotation))
      throw std::invalid_argument("invalid Ceres Pinocchio TCP transform");
...
CeresPinocchioArmKinematics::CeresPinocchioArmKinematics(
...
CeresPinocchioArmKinematics::~CeresPinocchioArmKinematics() = default;
CeresPinocchioArmKinematics::CeresPinocchioArmKinematics(
    CeresPinocchioArmKinematics&&) noexcept = default;
CeresPinocchioArmKinematics& CeresPinocchioArmKinematics::operator=(
    CeresPinocchioArmKinematics&&) noexcept = default;

int CeresPinocchioArmKinematics::configurationSize() const noexcept {
  return impl_->model.nq;
}

int CeresPinocchioArmKinematics::velocitySize() const noexcept {
  return impl_->model.nv;
}

ArmLimits CeresPinocchioArmKinematics::limits(ArmSide side) const {
  return impl_->limits(side);
}

ArmKinematicSample CeresPinocchioArmKinematics::sample(ArmSide side,
                                                  const Vec7& q) {
  return impl_->sample(side, q);
}

ArmKinematicSample CeresPinocchioArmKinematics::sampleTcp(ArmSide side,
                                                     const Vec7& q) {
