...
enum class PicoEeTargetSource { kPacketTargets, kMappedCorrectedPalm };
enum class PicoArmAngleReferenceSource {
  kPacketDirection,
  kMappedSkeletonPlane,
};
enum class PicoMappedArmAngleVectorMode { kDisabled, kShadow, kActive };
enum class PicoMappedVectorQpWeightMode { kLegacy, kDecoupled };
enum class VelocityQpSmoothnessMode { kLegacy, kNormalizedSplit };
enum class PicoEeTwistEstimatorMode { kLegacy, kShadow, kActive };
enum class PicoEeFeedforwardAllocatorMode { kLegacy, kShadow, kActive };
enum class PicoEeTaskAllocationMode { kDisabled, kShadow, kActive };
enum class IkAlgorithm {
  ...
  kPicoEeMappedCorrectedPalmVelocityQp,
};

inline bool usesPicoEeMappedCorrectedPalm(
    IkAlgorithm algorithm) noexcept {
  return algorithm == IkAlgorithm::kPicoEeMappedCorrectedPalmVelocityQp;
}
...
inline bool usesHierarchicalVelocityQp(IkAlgorithm algorithm) noexcept {
  return algorithm == IkAlgorithm::kHierarchicalQp ||
         ...
         usesPicoEeMappedCorrectedPalm(algorithm);
}
...
struct CartesianServoConfig {
  ...
  bool motion_gain_enabled{false};
  Eigen::Vector3d kp_position_motion{Eigen::Vector3d::Constant(4.0)};
  Eigen::Vector3d kp_orientation_motion{Eigen::Vector3d::Constant(3.0)};
...
struct PicoEeMotionConfidenceConfig {
  bool enabled{false};
  double linear_quiet_m_s{0.01};
  double linear_tracking_m_s{0.08};
  double angular_quiet_rad_s{0.05};
  double angular_tracking_rad_s{0.40};
...
struct HierarchicalQpConfig {
  ...
  VelocityQpSmoothnessMode smoothness_mode{
      VelocityQpSmoothnessMode::kLegacy};
  double full_acceleration_weight{0.0};
  double full_jerk_weight{0.0};
  Vec7 full_acceleration_scale_rad_s2{Vec7::Ones()};
  Vec7 full_jerk_scale_rad_s3{Vec7::Ones()};
  double nullspace_acceleration_weight{0.0};
  double nullspace_jerk_weight{0.0};
  double nullspace_acceleration_scale_rad_s2{1.0};
  double nullspace_jerk_scale_rad_s3{1.0};
  double nominal_gain{0.2};
...
struct PicoEeHeadroomConfig {
  double low_frequency_min_scale{0.0};
  double high_frequency_min_scale{0.0};
  bool redundancy_authority_enabled{false};
  double redundancy_zero_headroom{0.10};
  double redundancy_full_headroom{0.30};
};

struct CausalSe3TwistEstimatorConfig {
  PicoEeTwistEstimatorMode mode{PicoEeTwistEstimatorMode::kLegacy};
  int fit_window{5};
  double maximum_condition_number{1.0e8};
  double position_full_confidence_residual_m{0.005};
  double position_invalid_residual_m{0.020};
  double orientation_full_confidence_residual_rad{0.020};
  double orientation_invalid_residual_rad{0.100};
  double age_full_confidence_seconds{0.005};
  double maximum_age_seconds{0.020};
  double confidence_attack_seconds{0.050};
  double confidence_release_seconds{0.010};
  double extra_lead_seconds{0.0};
};

struct CartesianFeedforwardAllocatorConfig {
  PicoEeFeedforwardAllocatorMode mode{
      PicoEeFeedforwardAllocatorMode::kLegacy};
  int bisection_steps{3};
  int max_preview_solves{9};
  double maximum_scale_increase_per_cycle{0.25};
  double maximum_soft_bound_usage{0.95};
  double task_scale_tolerance{1.0e-6};
  double normalized_residual_tolerance{1.0e-4};
  double normalized_residual_floor{1.0e-3};
  double arm_angle_residual_tolerance{1.0e-4};
  double maximum_nullspace_leakage{1.0e-8};
  double preview_budget_seconds_per_arm{0.00075};
  double mismatch_p99_tolerance_rad_s{1.0e-5};
  double mismatch_max_tolerance_rad_s{1.0e-4};
};

// Upstream, constraint-aware scaling for the complete Cartesian task demand.
// This is deliberately separate from the final Velocity-Level QP: it only
// previews the existing solver and may authorize a smaller desired twist.
struct CartesianTaskAllocationConfig {
  PicoEeTaskAllocationMode mode{PicoEeTaskAllocationMode::kDisabled};
  // When a feedback twist is supplied, keep it unscaled and scale only the
  // remaining Cartesian feedforward component.
  bool preserve_feedback{true};
  // When enabled, keep the Cartesian demand unchanged and use the preview
  // scale only for the active vector-nullspace (redundancy) task.
  bool nullspace_only{false};
  // Optional preview search over a small set of nullspace rate candidates.
  // This is deliberately opt-in and has no effect on Cartesian task mode.
  bool nullspace_search_enabled{false};
  double nullspace_search_step{0.05};
  double nullspace_reference_weight{1.0};
  double nullspace_continuity_weight{1.0};
  double nullspace_jerk_weight{0.1};
  double nullspace_arm_rate_weight{1.0};
  int bisection_steps{4};
  int max_preview_solves{9};
  double minimum_task_scale{0.50};
  double minimum_preview_task_scale{0.995};
  double maximum_soft_bound_usage{0.95};
  double normalized_residual_tolerance{0.05};
  double preview_budget_seconds_per_arm{0.00075};
};

struct PicoMappedArmAngleVectorConfig {
  PicoMappedArmAngleVectorMode mode{PicoMappedArmAngleVectorMode::kDisabled};
  PicoMappedArmAngleVectorGovernorConfig governor;
  // Legacy forwards the complete governor objective curvature into the final
  // QP. Decoupled keeps reference shaping and final QP authority independent.
  PicoMappedVectorQpWeightMode qp_weight_mode{
      PicoMappedVectorQpWeightMode::kLegacy};
  double qp_tracking_weight{1.0};
  double qp_continuity_weight{1.0e-3};
  // Direct coefficient trend cost in the Velocity QP. This is intentionally
  // separate from governor.jerk_weight, whose normalized rollout term carries
  // dt^-4 scaling.
  double qp_jerk_weight{0.1};
};
...
struct QpIkConfig {
  ...
  PicoEeHeadroomConfig pico_ee_headroom;
  PicoEeMotionConfidenceConfig pico_ee_motion_confidence;
  CausalSe3TwistEstimatorConfig pico_ee_twist_estimator;
  CartesianFeedforwardAllocatorConfig pico_ee_feedforward_allocator;
  CartesianTaskAllocationConfig pico_ee_task_allocator;
  PicoMappedArmAngleVectorConfig pico_mapped_arm_angle_vector_nullspace;
