#include "tianji_mapped_palm/hierarchical_qp_ik.hpp"
...
ArmIkResult HierarchicalQpIk7::solve(const ArmIkInput& input) {
  ...
  const VelocityQpSmoothnessCosts smoothness =
      evaluateVelocityQpSmoothnessCosts(config_, input, result.qdot);
  result.smoothness_full_acceleration_cost = smoothness.full_acceleration;
  result.smoothness_full_jerk_cost = smoothness.full_jerk;
  result.smoothness_nullspace_acceleration_cost =
      smoothness.nullspace_acceleration;
  result.smoothness_nullspace_jerk_cost = smoothness.nullspace_jerk;
