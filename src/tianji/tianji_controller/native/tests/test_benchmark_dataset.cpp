...
// Config lives with the controller; models live in the description package.
...
std::string modelPath(const std::string& name) {
  return std::string(TIANJI_MODEL_DIR) + "/" + name;
}
...
TEST(BenchmarkDatasetTest, SafeSingularFixtureIsInteriorAndRankDeficient) {
  ...
  MujocoRobot robot(modelPath("marvin_m6_qp_test.xml"));
...
TEST(BenchmarkDatasetTest, IsDeterministicBalancedAndFinite) {
  ...
  const std::string model = modelPath("marvin_m6_qp_test.xml");
