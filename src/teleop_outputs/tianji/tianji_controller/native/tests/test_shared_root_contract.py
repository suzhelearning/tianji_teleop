...
def test_frozen_contract_and_models_match():
    result = validator.validate(controller_profile("qp_ik_pico_shared_root_dls.yaml"))
    assert result["passed"] and not result["motion_authorized"]


@pytest.mark.parametrize("mutation", ["hash", "origin", "mix", "sources", "simple_basis", "closure_axis", "closure_frame", "closure_evidence"])
def test_rejects_invalid_contract(tmp_path, mutation):
    profile = yaml.safe_load(controller_profile("qp_ik_pico_shared_root_dls.yaml").read_text())
    contract = yaml.safe_load(controller_profile("shared_root_tjvr_input_contract.yaml").read_text())
    geometry_path = controller_profile("shared_root_robot_geometry_dls.yaml")
    ...
    (tmp_path / "shared_root_robot_geometry_dls.yaml").write_text(yaml.safe_dump(geometry))
