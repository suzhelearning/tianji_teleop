...
TEST(SharedRootSkeletonScaler, ReconstructsRobotRestSkeletonOnFirstFrame) {
  const SharedRootRobotGeometry geometry = testGeometry();
  SharedRootSkeletonScaler scaler(geometry, testConfig());
  ...
  const SharedRootTargets result =
  ...
  EXPECT_EQ(result.left.upper_source, SharedRootSegmentSource::kRotation);
  EXPECT_EQ(result.left.forearm_source, SharedRootSegmentSource::kRotation);
  EXPECT_EQ(result.left.hand_source, SharedRootSegmentSource::kRotation);
...
TEST(SharedRootSkeletonScaler, AlignsPicoAndRobotJointFrameBasesImmediately) {
  SharedRootRobotGeometry geometry = testGeometry();
  ...
  const SharedRootTargets first = scaler.update(skeleton, palm, palm);
  ...
  const SharedRootTargets second = scaler.update(skeleton, palm, palm);
...
TEST(SharedRootSkeletonScaler, FallsBackPerSegmentToPositionDirection) {
  SharedRootSkeletonScaler scaler(testGeometry(), testConfig());
  auto skeleton = makeSkeleton();
  skeleton.rotations_valid = false;
  const Pose palm = makePalm(Eigen::Vector3d::Zero(), 0.0);

  const SharedRootTargets result = scaler.update(skeleton, palm, palm);

  ASSERT_TRUE(result.valid) << result.detail;
  EXPECT_EQ(result.left.upper_source, SharedRootSegmentSource::kPositionFallback);
  EXPECT_EQ(result.left.forearm_source,
            SharedRootSegmentSource::kPositionFallback);
  EXPECT_EQ(result.left.hand_source, SharedRootSegmentSource::kPositionFallback);
}

TEST(SharedRootSkeletonScaler, AlignsShortWristToPalmWhenRotationIsValid) {
  SharedRootSkeletonScaler scaler(testGeometry(), testConfig());
  auto skeleton = makeSkeleton();
  skeleton.points[3] =
...
TEST(SharedRootSkeletonScaler, RejectsWhenRotationAndPositionAreBothInvalid) {
  SharedRootSkeletonScaler scaler(testGeometry(), testConfig());
  auto skeleton = makeSkeleton();
  skeleton.rotations_valid = false;
...
TEST(SharedRootSkeletonScaler, CommitsFrameAlignmentBilaterally) {
  SharedRootSkeletonScaler scaler(testGeometry(), testConfig());
  const Pose palm = makePalm(Eigen::Vector3d::Zero(), 0.0);
  ...
  valid.rotations[0] = Eigen::Quaterniond(
      Eigen::AngleAxisd(0.4, Eigen::Vector3d::UnitZ()));
  const SharedRootTargets result = scaler.update(valid, palm, palm);
  ASSERT_TRUE(result.valid) << result.detail;
