...
std::string modelPath() {
  return (std::filesystem::path(TIANJI_MODEL_DIR) / "marvin_m6_qp_test.xml")
      .string();
}

std::string picoFastModelPath() {
  return (std::filesystem::path(TIANJI_MODEL_DIR) / "marvin_m6_qp_pico_fast.xml")
      .string();
}

std::string fullWujiModelPath() {
  return (std::filesystem::path(TIANJI_MODEL_DIR) / "marvin_m6_wuji2.xml")
      .string();
}
