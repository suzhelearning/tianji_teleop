...
RuckigTrajectoryLimiter7::RuckigTrajectoryLimiter7(
...
bool RuckigTrajectoryLimiter7::validState(
...
bool RuckigTrajectoryLimiter7::reset(const ArmMotionState& state) noexcept {
  if (!canReset(state)) {
    return false;
  }
  state_ = state;
...
bool RuckigTrajectoryLimiter7::canReset(
    const ArmMotionState& state) const noexcept {
  return validState(state);
}

bool RuckigTrajectoryLimiter7::beginSoftStart(SimulationSoftStartLimits limits) noexcept {
...
RuckigTrajectoryResult RuckigTrajectoryLimiter7::update(const Vec7& target,
  ...
  if (!preserve_constraints_ && candidate.q.allFinite()) {
  ...
  if (!preserve_constraints_ && candidate.qdot.allFinite()) {
