...
bool finiteGeometry(const SharedRootRobotGeometry& geometry) {
...
struct SegmentSelection {
  ...
  SharedRootSegmentSource source{SharedRootSegmentSource::kInvalid};
...
SegmentSelection selectSegment(
  ...
  if (skeleton.rotations_valid &&
    ...
    if (continuous) {
      ...
      if (finiteVector(result.vector)) {
        result.valid = true;
        result.source = SharedRootSegmentSource::kRotation;
  ...
  result.source = result.valid ? SharedRootSegmentSource::kPositionFallback
                               : SharedRootSegmentSource::kInvalid;
  return result;
}
...
std::string_view toString(SharedRootSegmentSource source) noexcept {
  switch (source) {
    case SharedRootSegmentSource::kInvalid:
      return "invalid";
    case SharedRootSegmentSource::kRotation:
      return "rotation";
    case SharedRootSegmentSource::kPositionFallback:
      return "position_fallback";
  }
  return "invalid";
}

SharedRootSkeletonScaler::SharedRootSkeletonScaler(
    SharedRootRobotGeometry geometry, SharedRootShapeConfig config)
    : geometry_(std::move(geometry)), config_(std::move(config)) {
  if (!finiteGeometry(geometry_)) {
    throw std::invalid_argument("Shared-root robot geometry is invalid");
  }
  if (!std::isfinite(config_.maximum_joint_rotation_jump_rad) ||
    ...
    throw std::invalid_argument("Shared-root rotation jump limit is invalid");
  }
  reset();
}

void SharedRootSkeletonScaler::reset() noexcept {
...
bool SharedRootSkeletonScaler::validInput(
...
SharedRootSkeletonScaler::ArmBuildResult SharedRootSkeletonScaler::makeTarget(
  ...
  result.valid = true;
  result.detail = "shared_root_shape_valid";
  return result;
}

void SharedRootSkeletonScaler::commitRotations(
    ArmSide side, const PicoUpperLimbSkeleton& skeleton,
...
SharedRootTargets SharedRootSkeletonScaler::update(
    const PicoUpperLimbSkeleton& skeleton, const Pose& left_palm,
    const Pose& right_palm) {
  SharedRootTargets result;
