...
ArmMapping MujocoRobot::buildMapping(ArmSide side) const {
  ...
  const std::string hand_tcp_frame = handTcpFrameSiteName(side);
  result.tcp_site_id =
      mj_name2id(model_, mjOBJ_SITE, hand_tcp_frame.c_str());
  if (result.tcp_site_id < 0) {
    result.tcp_frame_name = tcpSiteName(side);
    result.tcp_site_id = mj_name2id(model_, mjOBJ_SITE,
                                    result.tcp_frame_name.c_str());
  } else {
    result.tcp_frame_name = hand_tcp_frame;
  }
  if (result.tcp_site_id < 0) {
...
const std::string& MujocoRobot::tcpFrameName(ArmSide side) const noexcept {
  return mapping(side).tcp_frame_name;
}

bool MujocoRobot::usesHandTcpFrame(ArmSide side) const noexcept {
  return tcpFrameName(side) == handTcpFrameSiteName(side);
}
