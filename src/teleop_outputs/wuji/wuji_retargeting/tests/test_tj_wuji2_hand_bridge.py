...
from example import tj_wuji2_hand_bridge as bridge
from tianji import hand_protocol
from tianji_runtime import ResourceNotFound, package_share, workspace


def official_hand2_urdf(side):
    """Hand2 URDF shipped as package data of the installed wuji_retargeting."""
    import wuji_retargeting

    return (Path(wuji_retargeting.__file__).resolve().parent / "wuji-description"
            / "hand2" / "hand2_beta1" / "body" / "urdf" / f"{side}.urdf")


def viewer_model():
    """Viewer model whose joint limits every Hand2 joint must fit.

    ``tianji_description`` owns the model; the checkout copy is only a fallback
    while that package is not built in this environment.
    """
    try:
        return package_share("tianji_description", "models", "marvin_m6_wuji2.xml")
    except ResourceNotFound:
        candidate = workspace() / "src/teleop_outputs/tianji/tianji_description/models/marvin_m6_wuji2.xml"
...
class PacketTests(unittest.TestCase):
    ...
    def test_official_model_limits_fit_viewer_for_every_named_joint(self):
        ...
        for side in ("left", "right"):
            model = ET.parse(official_hand2_urdf(side))
...
class ModelWristFrameTests(unittest.TestCase):
    ...
    def test_rigid_hand_motion_preserves_model_wrist_targets(self):
        ...
        for side in ("left", "right"):
            first = bridge.HandRetargeter(side)
            second = bridge.HandRetargeter(side)
