from tianji_runtime.resources import package_share
import unittest
import types
...
class DirectTests(unittest.TestCase):
    def setUp(self):
        ...
        self.sim = DirectStateSimulation(DESCRIPTION/'mapped_palm/assets/mapped_palm/marvin_m6_wuji2.xml',
                          DESCRIPTION/'config/deployment.yaml')
    ...
    def test_home_with_actual_mujoco_state_stops_at_wait_for_ack(self):
        ...
        recovery.key(ord('H'))
        peak_velocity=0.
        with patch('simulation.mapped_recovery.os.write',side_effect=lambda fd,data:sent.append(data) or len(data)):
            for tick in range(3100):
                with patch('simulation.mapped_recovery.time.monotonic_ns',return_value=1_000_000_000+tick*1_000_000):
                    recovery.update()
                self.sim.step()
