...
from ..pico_owned_session import run_with_owned_pico, terminate_group, release_input
...
class OwnedPicoTests(unittest.TestCase):
    ...
    def execute(self, codes, cleanup_code=0):
        ...
        with patch('simulation.pico_owned_session.subprocess.Popen', side_effect=children) as start, \
             patch('simulation.pico_owned_session.release_input', return_value=cleanup_code) as stop:
    ...
    def test_sigint_waits_for_child_then_cleans(self):
        ...
        with patch('simulation.pico_owned_session.signal.signal', side_effect=install), \
             patch('simulation.pico_owned_session.os.killpg') as kill, \
             patch('simulation.pico_owned_session.subprocess.Popen', return_value=child) as start, \
             patch('simulation.pico_owned_session.release_input', return_value=0) as stop:
    ...
    def test_group_escalation_is_bounded(self):
        ...
        with patch('simulation.pico_owned_session.os.killpg') as kill:
            self.assertEqual(terminate_group(child), -signal.SIGKILL)
    ...
    def test_cleanup_timeout_terminates_helper_group(self):
        ...
        with patch('simulation.pico_owned_session.subprocess.Popen', return_value=child), \
             patch('simulation.pico_owned_session.terminate_group') as terminate:
    ...
    def test_ignored_interrupt_reaches_escalation_and_cleanup(self):
        ...
        with patch('simulation.pico_owned_session.signal.signal', side_effect=install), \
             patch('simulation.pico_owned_session.time.monotonic', side_effect=[0, 6]), \
             patch('simulation.pico_owned_session.os.killpg'), \
             patch('simulation.pico_owned_session.subprocess.Popen', return_value=child), \
             patch('simulation.pico_owned_session.terminate_group', side_effect=terminate) as escalation, \
             patch('simulation.pico_owned_session.release_input', return_value=0) as cleanup:
