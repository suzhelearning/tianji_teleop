...
class DataCollector(Node):
    def __init__(self, config_path=None):
        super().__init__('pico_recorder')
        configured_path = self.declare_parameter('config_path', config_path or '').value
        if not configured_path:
            configured_path = str(
                Path(get_package_share_directory('pico_recorder')) / 'config' / 'collect_config.yaml'
            )
        self.config = load_topics_config(configured_path)
...
def main(args=None):
    parser = argparse.ArgumentParser(description='Record PICO streams as HDF5 and MP4.')
    parser.add_argument('--config', help='Recorder YAML (defaults to installed package config).')
    options = parser.parse_args(remove_ros_args(args=args)[1:])
    rclpy.init(args=args)
    node = None
    spin_thread = None
    try:
        node = DataCollector(options.config)
...
def keyboard_main(args=None):
    """Publish the original PICO record flag from a separate interactive terminal."""
    if not sys.stdin.isatty():
        raise SystemExit('pico_recorder_keyboard requires an interactive terminal')
    rclpy.init(args=args)
    node = rclpy.create_node('pico_recorder_keyboard')
    topic = node.declare_parameter('record_flag_topic', '/pico/record_flag').value
    publisher = node.create_publisher(Bool, topic, 10)
    print('Keys: [s] start collecting  [d] stop collecting  [q] quit keyboard')
    try:
        while rclpy.ok():
            ch = getch()
            if ch in ('q', '\x03'):
                break
            if ch in ('s', 'd'):
                publisher.publish(Bool(data=ch == 's'))
    except (KeyboardInterrupt, ExternalShutdownException):
        pass
    finally:
        node.destroy_node()
