...
from manus_bridge.start_hand_teleop import check_collector


COLLECTOR = Path('/fixture/rawviz')
SDK = Path('/fixture/libManusSDK_Integrated.so')
GOOD = f'libManusSDK_Integrated.so => {SDK} (0x1000)\n'


def test_no_execute_permission():
    with patch('manus_bridge.start_hand_teleop.os.access', return_value=False), \
         patch('manus_bridge.start_hand_teleop.subprocess.run') as run:
...
def test_lfs_pointer_rejected_without_loader():
    with patch('manus_bridge.start_hand_teleop.os.access', return_value=True), \
...
def test_loader_results(output, code, error):
    with patch('manus_bridge.start_hand_teleop.os.access', return_value=True), \
         ...
         patch('manus_bridge.start_hand_teleop.subprocess.run',
...
def test_loader_timeout():
    with patch('manus_bridge.start_hand_teleop.os.access', return_value=True), \
         patch.object(Path, 'open', side_effect=lambda *_: io.BytesIO(b'\x7fELF')), \
         patch('manus_bridge.start_hand_teleop.subprocess.run', side_effect=subprocess.TimeoutExpired('ldd', 5)):
