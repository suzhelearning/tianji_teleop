...
int main(int argc, char *argv[])
{
    ...
    #ifdef ROS2
        // Local integration: let the launch layer override the control config.
        // start_full_body.sh --odin-map writes a temp control_command.yaml (custom_map_mode=2 +
        // relocalization_map_abs_path) and passes it via the `config_file` parameter; honor it so
        // the map/relocalization mode is actually picked up. Empty/missing => in-tree default above.
        {
            std::string config_file_param = node->declare_parameter<std::string>("config_file", "");
            if (!config_file_param.empty()) {
                if (std::filesystem::exists(config_file_param)) {
                    config_file = config_file_param;
                    RCLCPP_INFO(node->get_logger(),
                        "Using config_file override from parameter: %s", config_file.c_str());
                } else {
                    RCLCPP_WARN(node->get_logger(),
                        "config_file parameter '%s' not found; falling back to default: %s",
                        config_file_param.c_str(), config_file.c_str());
                }
            }
        }
    #endif
