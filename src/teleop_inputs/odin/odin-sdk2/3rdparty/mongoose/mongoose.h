...
static inline void mg_free(void *ptr) { vPortFree(ptr); }
...
#define MG_SOCK_PENDING(errcode)                                                               \
  (((errcode) < 0) && (WSAGetLastError() == WSAEINTR || WSAGetLastError() == WSAEINPROGRESS || \
...
#define MG_IPV6(a, b, c, d, e, f, g, h)                                                            \
  {                                                                                                \
    (uint8_t)((a) >> 8), (uint8_t)(a), (uint8_t)((b) >> 8), (uint8_t)(b), (uint8_t)((c) >> 8),     \
        (uint8_t)(c), (uint8_t)((d) >> 8), (uint8_t)(d), (uint8_t)((e) >> 8), (uint8_t)(e),        \
        (uint8_t)((f) >> 8), (uint8_t)(f), (uint8_t)((g) >> 8), (uint8_t)(g), (uint8_t)((h) >> 8), \
        (uint8_t)(h)                                                                               \
  }
...
#define MG_LOAD_BE32(p)                                                           \
  ((uint32_t)(((uint32_t)MG_U8P(p)[0] << 24U) | ((uint32_t)MG_U8P(p)[1] << 16U) | \
              ((uint32_t)MG_U8P(p)[2] << 8U) | MG_U8P(p)[3]))
#define MG_LOAD_BE64(p)                                                           \
  ((uint64_t)(((uint64_t)MG_U8P(p)[0] << 56U) | ((uint64_t)MG_U8P(p)[1] << 48U) | \
              ((uint64_t)MG_U8P(p)[2] << 40U) | ((uint64_t)MG_U8P(p)[3] << 32U) | \
              ((uint64_t)MG_U8P(p)[4] << 24U) | ((uint64_t)MG_U8P(p)[5] << 16U) | \
...
struct mg_connection *mg_listen(struct mg_mgr *, const char *url, mg_event_handler_t fn,
                                void *fn_data);
struct mg_connection *mg_connect(struct mg_mgr *, const char *url, mg_event_handler_t fn,
                                 void *fn_data);
...
struct mg_connection *mg_http_listen(struct mg_mgr *, const char *url, mg_event_handler_t fn,
                                     void *fn_data);
struct mg_connection *mg_http_connect(struct mg_mgr *, const char *url, mg_event_handler_t fn,
                                      void *fn_data);
...
typedef struct MG_UECC_HashContext {
  ...
  void (*update_hash)(const struct MG_UECC_HashContext *context, const uint8_t *message,
                      unsigned message_size);
...
mg_uecc_word_t mg_uecc_vli_equal(const mg_uecc_word_t *left, const mg_uecc_word_t *right,
...
struct mg_connection *mg_ws_connect(struct mg_mgr *, const char *url, mg_event_handler_t fn,
...
struct mg_tcpip_sdio {
  ...
  bool (*xfr)(void *, bool write, uint32_t arg, uint16_t blksz, uint32_t *, uint32_t len,
              uint32_t *r);
};
