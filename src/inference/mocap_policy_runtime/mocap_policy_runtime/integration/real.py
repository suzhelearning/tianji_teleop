...
from tianji_controller.protocol import CommandFrame, DEVICE_READY_FLAGS
from tianji_controller.safety import MotionGate, SafetyFault, _BOUND_SLACK_RAD
from tianji_controller.staged_motion import ALIGNING, READY, StagedMotionGate
from tianji_controller.viewer import RealRobotViewer
from tianji_runtime.resources import config_path as config_file, workspace
...
class StableLocalization:
    ...
    def __init__(self, position_limit, angle_limit, stable_s, speed_limit, feedback_timeout_ns, devices):
        from tianji_controller.settling import FeedbackRest
...
def run(args):
    ...
    if not sys.stdin.isatty():
        ...
        return 2
    hardware = {}
    keyboard = ik = tracker = session = viewer = adapter = None
    handlers = {}
    result = 1
    try:
        ...
        try:
            # The executor is an ament package: it is only importable from a
            # built workspace overlay, so defer it to the real-hardware path
            # instead of making this module fail to import.
            from tianji_controller.run_teleop import (
                fresh_feedback, load_configuration, make_hardware, resolve,
            )
        except ImportError as error:
            raise RuntimeError(
                f"tianji_controller is not importable ({error}); "
                "build the workspace with: pixi run build"
            ) from error
        ...
        model_path = resolve(workspace(), config["controller_model"])
        ...
        for name in devices:
            hardware.update(make_hardware(config, (name,), workspace()))
        ...
        for name, resource in (("Motive tracker", tracker), ("Motive session", session),
                               ("target adapter", adapter), ("native IK", ik),
                               ("visualization", viewer), ("keyboard", keyboard)):
...
def parser():
    ...
    result.add_argument("--config", type=Path, default=config_file("robot.json"))
    result.add_argument("--policy-config", type=Path, default=PACKAGE_ROOT / "configs/regrind.yaml")
    result.add_argument("--replay-config", type=Path, default=PACKAGE_ROOT / "configs/replay.yaml")
