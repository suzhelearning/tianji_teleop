...
from tianji_controller import hardware, run_teleop
...
def _h5_source(monkeypatch, path, mode):
    ...
    if mode == "retargeted":
        ...
        monkeypatch.setattr(hand, "HandRetargeter", Retargeter)
...
def test_h5_real_rejects_unsafe_prepared_hand_before_hardware(monkeypatch, tmp_path, mode, bad_value):
    ...
    monkeypatch.setattr(run_teleop, "make_hardware", lambda *a: pytest.fail("hardware opened before hand preflight"))
...
def _exercise_source(monkeypatch, tmp_path, *, lose_motive=False, lose_object=False,
    ...
    config, _ = run_teleop.load_configuration(config_path("robot.json"))
    ...
    monkeypatch.setattr(run_teleop, "make_hardware", make_hardware)
    ...
    if policy_mode:
        ...
        profile = yaml.safe_load((real.PACKAGE_ROOT / "configs/regrind.yaml").read_text())
...
def test_right_impedance_verifies_echo_and_never_switches_left(monkeypatch):
    from tianji_controller.tests.test_hardware import Clock, FakeMarvin
    ...
    settings = real._policy_settings(real.PACKAGE_ROOT / "configs/regrind.yaml")
    try:
        device.connect()
        device.enable()
