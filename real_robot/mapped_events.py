...
def decode_event(data, token):
    ...
    if magic!=b'MRE1' or version not in (1,2) or size!=len(data) or state not in range(1,9) or identity!=token.encode('ascii'):
        raise SafetyFault('invalid event header/identity')
    extra={}
    offset=HEADER.size
    if version==2:
        if len(data)!=HEADER.size+CALIBRATION.size+PACKET_SIZE: raise SafetyFault('invalid calibration event size')
...
class EventReceiver:
    def __init__(self,reserved,*,bounded=True,calibration=False):
    ...
    def arguments(self):
        return ['--real-event-fd',str(self.writer.fileno()),'--real-event-token',self.token]+(
            ['--real-resync-bounded'] if self.bounded else [])+(['--real-xz-calibration'] if self.calibration else [])
    ...
    def drain(self,validate=None):
        for _ in range(512):
            ...
            if self.calibration and data[4]!=2: raise SafetyFault('calibration status missing')
