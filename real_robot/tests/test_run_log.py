...
from real_robot import run_log
from real_robot.run_log import SessionLog
from real_robot.run_teleop import main
from real_robot.tests.test_safety import NOW, feedback, frame
...
class EnvironmentTests(unittest.TestCase):
    ...
    def test_unexpected_exception_is_recorded_as_nonzero_without_swallowing_it(self):
        with tempfile.TemporaryDirectory() as folder:
            with patch.dict(os.environ, {run_log.LOG_DIR_ENV: folder}), \
                    patch("real_robot.run_teleop.make_hardware", side_effect=TypeError("unexpected failure")), \
                    redirect_stdout(io.StringIO()), redirect_stderr(io.StringIO()):
    ...
    def test_controller_cleanup_failure_does_not_erase_the_original_failure_log(self):
        with tempfile.TemporaryDirectory() as folder:
            with patch.dict(os.environ, {run_log.LOG_DIR_ENV: folder}), \
                    patch("real_robot.run_teleop.make_hardware", side_effect=RuntimeError("original failure")), \
                    patch("real_robot.run_teleop.stop_controller", side_effect=OSError("cleanup failed")), \
                    redirect_stdout(io.StringIO()), redirect_stderr(io.StringIO()):
    ...
    def test_inspection_reports_failure_when_the_final_log_cannot_be_saved(self):
        ...
        from real_robot.hardware import Feedback

        device = Mock()
        device.read_feedback.side_effect = lambda: Feedback((0.,) * 20, time.monotonic_ns(), True, False)
        with tempfile.TemporaryDirectory() as folder:
            with patch.dict(os.environ, {run_log.LOG_DIR_ENV: folder}), \
                    patch("real_robot.run_teleop.make_hardware", return_value={"right_hand": device}), \
    ...
    def test_unwritable_log_directory_refuses_before_hardware(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            with patch.dict(os.environ, {run_log.LOG_DIR_ENV: str(blocker / "logs")}), \
                    patch("real_robot.run_teleop.make_hardware") as factory, \
                    redirect_stdout(io.StringIO()), redirect_stderr(errors):
                result = main(["--inspect", "--devices", "right_hand",
                               "--config", str(ROOT / "real_robot/config.json")])
            self.assertEqual(result, 1)
    ...
    def test_session_is_written_before_hardware_creation_and_keeps_the_failure(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            with patch.dict(os.environ, {run_log.LOG_DIR_ENV: str(log_dir)}), \
                    patch("real_robot.run_teleop.make_hardware", side_effect=RuntimeError("no hardware here")), \
                    redirect_stdout(io.StringIO()), redirect_stderr(io.StringIO()):
                result = main(["--inspect", "--devices", "right_hand",
                               "--config", str(ROOT / "real_robot/config.json")])
            self.assertEqual(result, 1)
            ...
            self.assertEqual(session["config_source"], str((ROOT / "real_robot/config.json").resolve()))
