...
class GeometryVisibilityTests(unittest.TestCase):
    def test_localization_places_robot_sites_static_meshes_and_data_in_one_frame(self):
        ...
        model_path = Path(__file__).resolve().parents[2] / "control/models/marvin_m6_wuji2.xml"
    ...
    def test_reference_hand_world_placement_is_independent_of_both_joint_streams(self):
        ...
        root = Path(__file__).resolve().parents[2]
        model = mujoco.MjModel.from_xml_path(str(root / "control/models/marvin_m6_wuji2.xml"))
        indices, ghost_geoms = _prepare_model(mujoco, model)
        actual, control = mujoco.MjData(model), mujoco.MjData(model)
        options = mujoco.MjvOption()
        overlay = ReferenceOverlay(model, mujoco, indices)
    ...
    def test_object_axes_are_independent_robot_world_poses_and_expire_without_update(self):
        ...
        scene = mujoco.MjvScene(model, maxgeom=1000)
        with patch("real_robot.viewer.time.monotonic_ns", return_value=NOW_NS):
            overlay.draw(scene)
        ...
        scene.ngeom = 0
        with patch("real_robot.viewer.time.monotonic_ns", return_value=NOW_NS + 100_000_001):
            overlay.draw(scene)
    ...
    def test_hammer_mesh_preserves_obj_frame_and_expires_only_actual(self):
        ...
        import numpy as np
        from scipy.spatial.transform import Rotation
        from control.model_assets import OBJECT_BODY_NAME, OBJECT_MESH_NAME
        from real_robot.viewer import (
    ...
    def test_explicit_missing_or_invalid_mesh_fails_instead_of_axes_fallback(self):
        import mujoco
        from real_robot.viewer import _viewer_model
    ...
    def test_reference_model_limits_are_rejected_not_clipped(self):
        import mujoco
        from real_robot.viewer import ReferenceOverlay, _prepare_model
...
class RenderThreadLifecycleTests(unittest.TestCase):
    def test_localization_recenters_once_then_preserves_operator_camera(self):
        ...
        import numpy as np
        from scipy.spatial.transform import Rotation
        from real_robot.viewer import CAMERA_LOOKAT, ReferenceOverlay, _prepare_model, _render_loop
        ...
        with patch("real_robot.viewer.sys.stdin") as stdin, \
                patch("real_robot.viewer.os.set_blocking"), \
                patch("real_robot.viewer.os.read", side_effect=messages), \
                patch("real_robot.viewer.time.monotonic_ns", return_value=NOW_NS), \
                patch("real_robot.viewer.time.sleep"):
