...
def safety_limits() -> tuple[np.ndarray, np.ndarray]:
    config = json.loads((REPOSITORY / "real_robot/config.json").read_text())
...
def configured_home() -> np.ndarray:
    site_file = REPOSITORY / "real_robot/config.json"
    site = json.loads(site_file.read_text())
    controller_path = (site_file.parent / site["controller_config"]).resolve()
...
class NativeIK:
    ...
    def __init__(self, model=None, config=None, initial_positions=None, *, binary=None,
        ...
        self.model_path = (Path(model) if model is not None else REPOSITORY / "control/models/marvin_m6_wuji2.xml").resolve()
        self.config_path = (Path(config) if config is not None else REPOSITORY / "control/config/qp_ik_cartesian_otg_velocity.yaml").resolve()
        executable = Path(binary) if binary is not None else REPOSITORY / "control/build/mocap_tcp_worker"
        for path in (self.model_path, self.config_path, executable):
            if not path.is_file():
                raise FileNotFoundError(f"{path}; build worker with: pixi run cmake --build control/build --target mocap_tcp_worker")
