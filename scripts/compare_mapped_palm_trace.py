...
def udp_exercise(records, bundle, recovery_test=False):
    ...
    from real_robot.protocol import decode_packet, ARMS_READY
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as output, socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as reserved:
        ...
        args = [str(ROOT/'control/build-mapped-palm/mapped_palm_tjrc_controller'),
...
def main():
    ...
    bundle=ROOT/'control/mapped_palm'
    ...
    result['asset_sha256'] = {
        ...
        for name,path in {
            ...
            'target_worker': ROOT/'control/build-mapped-palm/mapped_palm_native_worker',
            'target_adapter': ROOT/'control/build-mapped-palm/mapped_palm_tjrc_controller',
        }.items()
    }
    with args.report.open('x') as report:
        try:
            ...
            with ExitStack() as stack:
                ...
                new=stack.enter_context(MappedPalmWorkerClient(worker=ROOT/'control/build-mapped-palm/mapped_palm_native_worker',
                    model=bundle/'assets/mapped_palm/marvin_m6_wuji2.xml',urdf=bundle/'assets/mapped_palm/marvin_m6_s_ccs_696_v4_local.urdf',**common))
