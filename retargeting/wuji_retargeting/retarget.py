...
class Retargeter:
    ...
    def _apply_rotation(self, keypoints: np.ndarray) -> np.ndarray:
        ...
        Returns:
            Rotated keypoints
        """
        # Reuse the fixed transform on the frame path. Comparing the current
        # values also preserves live tuning and in-place edits of rotation_xyz.
        ...
        if angles != self._rotation_angles:
            from scipy.spatial.transform import Rotation

            self._rotation_matrix_t = Rotation.from_euler(
                'xyz', angles, degrees=True
            ).as_matrix().T
            self._rotation_angles = angles
        return keypoints @ self._rotation_matrix_t
