...
| Wuji Hand 2 | `adaptive_analytical_wuji_glove_wuji_hand_2_left.yaml` | `adaptive_analytical_wuji_glove_wuji_hand_2_right.yaml` |

> **务必确认改对了文件**：Wuji Hand 2 的配置在带 `wuji_hand_2` 的文件里，它通过 `optimizer.urdf_path / mjcf_path / link_naming` 指向 Wuji Hand 2 模型；改错文件（例如给 Wuji Hand 2 调参却编辑了默认 Wuji Hand 的 yaml）不会生效。下面 `tuning_tool.py` 的启动也要相应用 `--config` 指定 Wuji Hand 2 文件（见 §1）。
...
```bash
# 从 monorepo 根目录运行，使用根目录 .venv

# === Wuji Hand（默认手型）===
# 不带 --config 时，--wuji-glove 默认加载 config/adaptive_analytical_wuji_glove_<hand>.yaml
.venv/bin/python -m retargeting.example.tuning_tool --wuji-glove --hand right --glove-sn <YOUR_SN>
.venv/bin/python -m retargeting.example.tuning_tool --wuji-glove --hand left  --glove-sn <YOUR_SN>

# === Wuji Hand 2 ===
# 必须用 --config 显式指定 wuji_hand_2 文件，否则会回落到默认 Wuji Hand 配置
.venv/bin/python -m retargeting.example.tuning_tool --wuji-glove --hand right --glove-sn <YOUR_SN> \
  --config config/adaptive_analytical_wuji_glove_wuji_hand_2_right.yaml
.venv/bin/python -m retargeting.example.tuning_tool --wuji-glove --hand left  --glove-sn <YOUR_SN> \
  --config config/adaptive_analytical_wuji_glove_wuji_hand_2_left.yaml
```

> Wuji Hand 2 调参时，启动命令的 `--config` 与你编辑的 yaml **必须是同一个 wuji_hand_2 文件**，否则看到的是默认 Wuji Hand 的效果。

使用方法：

1. 启动 `tuning_tool.py`。
2. 打开**与启动命令一致**的那个 YAML：默认 Wuji Hand 用 `adaptive_analytical_wuji_glove_<hand>.yaml`，Wuji Hand 2 用 `adaptive_analytical_wuji_glove_wuji_hand_2_<hand>.yaml`。
...
| `mediapipe_rotation` | 输入 21 点整体旋转微调，单位度 | Wuji Hand：左手 `(5, -5, 20)`、右手 `(-5, -5, -20)`；Wuji Hand 2 因腕部 link 朝向不同，默认值见对应 `wuji_hand_2` 文件 | 用于纠正手套坐标系朝向；不同手型与左右手的默认值已分别写在各自 yaml 里 |
