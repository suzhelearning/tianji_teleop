...
def joint_permutation(joint_names, side: str) -> np.ndarray:
    ...
    expected = tuple(prefix + stem for stem in JOINT_STEMS)
...
class HandTargetCache:
    ...
    def sample(self):
        ...
        return encode_packet(sequence, timestamp_ns, left=left, right=right,
...
class HandRetargeter:
    ...
    def __init__(self, repository_root: Path, side: str):
        _side(side)
        hand_root = Path(repository_root).resolve()
        from retargeting.wuji_retargeting import Retargeter
        from scipy.spatial.transform import Rotation
        from retargeting.wuji_retargeting.mediapipe import (
            OPERATOR2MANO_LEFT, OPERATOR2MANO_RIGHT, estimate_frame_from_hand_points,
        )

        config = hand_root / "example/config" / f"retarget_manus_wuji_hand_2_{side}.yaml"
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--repository-root", type=Path, default=Path(__file__).resolve().parents[1],
                        help="owned retargeting source/assets directory containing example/ and wuji_retargeting/")
    ...
    models = {side: HandRetargeter(args.repository_root, side) for side in ("left", "right")}
