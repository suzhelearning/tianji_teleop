...
先按上文准备裸手 APK、手部 worker 和 Python 依赖，再在工程根目录执行：

```bash
# 构建主工程中的共享 DLS/Ruckig 与 PICO2 本地管道 worker
pixi run --locked build
bash pico2_sim.sh --mapping-mode shared-root --height-m 1.62
```

身高单位米，支持范围 `[1.0, 2.4]`；必须替换实际身高。
只测试双臂可加 `--disable-hands`，记录仍用 `--record FILE.h5`，文件独占创建。
不使用 VR 的人员档案、手柄 TCP 标定或 Manus；手部 retarget 路线保持原 PICO2 实现。

1. 在可跟踪范围内面向前方，双臂水平前伸、双手间距约肩宽、掌心相对。
2. 空闲时按 **C**，保持头部和双腕稳定约 1 秒；成功后等新帧再按 **S**。
   仍需至少 30 个有效独立采样，覆盖至少 0.75 秒；位置／姿态稳定性门限不变。
3. **H** 停止跟随、制动后平滑回双臂 Home，手指保持，不再走旧 V131 的 Python 五次回程。
4. **P／空格** 制动保持，也可取消 H 发起的回程（包括回程前制动阶段）；
   **Q／Ctrl+C** 回双臂 Home 后退出。退出流程开始后不再接受 P／空格，以免悬停阻塞退出。

未完成 C、标定采样中、失败、回程中或输入过期均不能 S。失败后需重做 C。
接收器断开会先制动并要求人工 S；输入连接代次变化还会清除 C，运行中则回 Home；
不能静默沿用旧坐标系。新连接重新 C 后，人工 S 在静止状态建立新的求解时间戳 epoch，允许输入程序重新计时；
同一连接的倒序输入仍拒绝，不在运动／制动中重置求解器。
跟踪失效或帧龄超过 45 ms 时先制动，不继续追逐旧目标。
仅由这种输入失效触发的保持可自动续接：从最后有效输入起 **1 秒内**，
同一连接恢复至少 **100 ms、5 个独立有效帧**（相邻帧间隔不超过 45 ms），
且映射仍有效。恢复资格成立后仍须等待原生制动完全结束，并有新的有效帧，
才走与 S 相同的软启动；制动等待期间再次失鲜则取消资格。
重复／倒序帧不更新恢复计数。超时、TCP 断开／重连、映射或 IK 拒绝以及主动
P/空格、H/Q、C/R/S 操作均取消自动恢复；P 在制动期间也会取消。
长时断流恢复后需要 S；连接身份变化还需重新 C。Home 和主动暂停不会自行接管。
Ruckig 仅用于双臂，手指依旧由原手部链路处理；Home 不保证手指回初始姿态。

### 输入与人体模板契约

PICO2 协议已有 wrist 与 palm 点，本模式只从 **wrist** 出发，
用 wrist→middle proximal（点 12）的向量确定掌长方向，添加一次估计腕掌距离；
**不使用 palm 点再添加偏移**。关键点失效或向量退化时拒绝该输入。
肩宽 `0.1828 H`，上臂 `0.155882 H`，前臂 `0.152941 H`，腕掌 `0.037037 H`。
这些都是声明的对称人体模板，不是个人实测，也不把腕掌距离称为实测“半掌长”。

C 从头显水平朝向锁定根轴，由前伸双掌中点减去人体模板臂展得到虚拟肩中点，
记录其相对头显位置；同时记录腕姿态到机器人参考掌心姿态的固定旋转。
机器人前伸参考由当前 DLS 模型的 FK 查询（J2=-90°、其余为零），不执行这个姿态。
运行时虚拟根随头显**平移**，不随转头旋转；躯干转向、明显低头弯腰后应 H 再 C。
仅靠头显和双手无法区分头部平移与躯干运动，这是模型限制。

掌心位置复用 `SharedRootTargetBuilder` 的共享根仿射公式：
`p_B = o_B + R_BCt diag(s_reach, s_width, s_reach) p_Ct`。
机器人根、肩宽及臂展读取仓库 `shared_root_robot_geometry_ceres.yaml`；
位置缩放不施加到姿态上，不改变机器人 TCP 或模型掌长。
**这里复用掌心坐标／尺度映射，不伪造实测肩肘，也未移植完整 VR 的 M0 骨架重建、
闭合投影和 shape guidance。**DLS 遇到远目标仍按现有控制器规则处理近似解或拒绝。

### IK、显示和验证边界

原生 worker 直接链接主工程 `DualArmController`，加载与
`bash teleop.sh --sim --user NEW_USER` 一样的 `qp_ik_pico_shared_root_dls.yaml`、
`marvin_m6_wuji2_shared_root_ceres.xml` 和配置指定的 Pinocchio URDF；
不是另外复制一套 DLS。200 Hz 名义周期在线执行 DLS 和 Ruckig，
复用软启动（接近后 0.5 秒恢复正常限值）及制动／Home 恢复逻辑。
本模式 S 接近阶段上限为速度 **1.4 rad/s**、加速度 **3 rad/s²**、jerk **12 rad/s³**，
仍取正常跟踪限值与上述上限的较小值；这是 PICO2 仿真专用覆盖，不修改 VR 的
0.35／0.5／2 默认软启动、正常跟踪限值或回 Home 参数。重启仿真后生效。
显示模型启动时校验实际 worker 的 **palm TCP**，不混用旧 V131 Base 系 flange TCP。

原始包与状态／C 参数写入现有录制事件，保留原 schema；退出成功排空前不标完整。
无硬件输出和真实运动权限。窗口仅显示机器人，不宣称已有虚拟人体全骨架叠加。
原 `--self-test` 属于 V131，不用于新模式；新模式离线回归：

```bash
PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 pixi run --locked python -m pytest pico2_hands/tests/test_shared_root.py -q
```

合成多身高、C 标定及状态机通过不等于真实输入精度或实时性验收；需要现场再核对。

### 按键和故障行为

以下按键与历史说明适用于**默认旧 V131 路线**，新模式按上节操作。

窗口聚焦时或启动终端都可按键，状态显示在窗口及终端：

PICO2 两种模式均在创建窗口前取消 MuJoCo 默认 **H→凸包显示**绑定，
H 只保留回 Home 回调，不再等显示同步后才撤销凸包，以避免底座膨胀闪帧。
实现只临时修改当前进程的导出 UI 快捷键表，退出时恢复；不修改安装文件、模型或其他进程。
当前实现针对仓库固定的 MuJoCo 3.10.0，有版本检查；其他版本须先审查适配，不能静默套用。
C 的接触点、S/R 的阴影／反射仍沿用同步后恢复策略，本轮未扩大修改范围。
实际现场窗口效果仍需重启验证。
