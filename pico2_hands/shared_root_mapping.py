...
class SharedRootMapping:
    def __init__(self, height_m, forward):
        ...
        artifact = ROOT / "control/config/shared_root_robot_geometry_ceres.yaml"
        ...
        for path_key, hash_key in (("urdf_path", "urdf_sha256"), ("mujoco_xml_path", "mujoco_xml_sha256")):
            if hashlib.sha256((artifact.parent / geometry[path_key]).read_bytes()).hexdigest() != geometry[hash_key]:
