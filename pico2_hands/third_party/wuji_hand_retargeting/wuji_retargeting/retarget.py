...
class Retargeter:
    ...
    def __init__(self, config: dict, hand_side: str = "right"):
        ...
        # Optional input-axis reflection/sign adjustment. This is applied to
        # raw keypoints before wrist-frame estimation so input devices with a
        # different handed coordinate convention can preserve chirality.
        self.input_axis_sign = np.asarray(
            retarget_config.get('input_axis_sign', [1.0, 1.0, 1.0]),
            dtype=np.float64,
        )
        if self.input_axis_sign.shape != (3,) or not np.isin(
            self.input_axis_sign, [-1.0, 1.0]
        ).all():
            raise ValueError(
                "retarget.input_axis_sign must contain three values, each +1 or -1"
            )
    ...
    def retarget(
        ...
        """
        mediapipe_kp = self._prepare_keypoints(raw_keypoints)
    ...
    def _prepare_keypoints(self, raw_keypoints: np.ndarray) -> np.ndarray:
        """Apply input chirality, wrist-frame, rotation, and offset transforms."""

        keypoints = np.asarray(raw_keypoints, dtype=np.float64)
        if keypoints.shape != (21, 3):
            raise ValueError(f"raw_keypoints must have shape (21, 3), got {keypoints.shape}")
        if not np.isfinite(keypoints).all():
            raise ValueError("raw_keypoints contain NaN or infinity")
        keypoints = keypoints * self.input_axis_sign
        mediapipe_kp = apply_mediapipe_transformations(
            keypoints,
            self.hand_side,
        )
