...
from pico2_hands.shared_root_mapping import SharedRootMapping, ROOT
...
class MappingTests(unittest.TestCase):
    ...
    def test_one_second_still_rejects_too_few_samples(self):
        ...
        self.assertEqual(m.error,"insufficient calibration samples")
...
@unittest.skipUnless((ROOT/"control/build/pico2_dls_worker").is_file(),"build main DLS worker")
class NativeTests(unittest.TestCase):
    def setUp(self):
        config=yaml.safe_load((ROOT/"control/config/qp_ik_pico_shared_root_dls.yaml").read_text())["controller"]
    ...
    def test_dls_start_brake_home_and_display(self):
        ...
        sim=DirectStateSimulation(ROOT/"control/models/marvin_m6_wuji2_shared_root_ceres.xml",ROOT/"control/config/qp_ik_pico_shared_root_dls.yaml")
    ...
    def test_cli_rejects_missing_height_and_legacy_height(self):
        for args in (["--mapping-mode","shared-root"],["--height-m","1.62"],
            ...
            result=subprocess.run([sys.executable,"-m","pico2_hands.run_sim",*args],cwd=ROOT,capture_output=True,timeout=10)
