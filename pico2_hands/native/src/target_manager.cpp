...
TargetManager::TargetManager(QpIkConfig config, DualArmTargets initial_targets)
    : config_(config),
      ...
      left_pre_ik_conditioner_(initial_targets.left,
                               config_.dexhand_velocity_qp.pre_ik_conditioning),
      right_pre_ik_conditioner_(initial_targets.right,
                                config_.dexhand_velocity_qp.pre_ik_conditioning) {}
...
void TargetManager::setMode(TargetMode mode, double start_time_seconds) {
  ...
  if (mode == TargetMode::kHold) {
    ...
    left_pre_ik_conditioner_.synchronize(last_output_.left);
    right_pre_ik_conditioner_.synchronize(last_output_.right);
  }
  mode_ = mode;
...
void TargetManager::setManualTarget(ArmSide side, const Pose& requested) {
  ...
  const Pose conditioned = conditionManualTarget(side, requested);
  target = limitManualIncrement(target, conditioned);
...
bool TargetManager::setManualTarget(ArmSide side, const Pose& requested,
  ...
  DexhandTargetConditioner conditioner_candidate =
      side == ArmSide::kLeft ? left_pre_ik_conditioner_
                             : right_pre_ik_conditioner_;
  const Pose conditioned = config_.dexhand_velocity_qp.pre_ik_conditioning.enabled
                               ? conditioner_candidate.condition(requested)
                               : requested;
  const Pose accepted = limitManualIncrement(target, conditioned);
  if (state.has_frame) {
  ...
  if (config_.dexhand_velocity_qp.pre_ik_conditioning.enabled) {
    if (side == ArmSide::kLeft) {
      left_pre_ik_conditioner_ = std::move(conditioner_candidate);
    } else {
      right_pre_ik_conditioner_ = std::move(conditioner_candidate);
    }
  }
  return true;
}
...
bool TargetManager::setManualTargets(const Pose& left_requested,
  ...
  DexhandTargetConditioner left_conditioner_candidate =
      left_pre_ik_conditioner_;
  DexhandTargetConditioner right_conditioner_candidate =
      right_pre_ik_conditioner_;
  const Pose conditioned_left =
      config_.dexhand_velocity_qp.pre_ik_conditioning.enabled
          ? left_conditioner_candidate.condition(left_requested)
          : left_requested;
  const Pose conditioned_right =
      config_.dexhand_velocity_qp.pre_ik_conditioning.enabled
          ? right_conditioner_candidate.condition(right_requested)
          : right_requested;
  if (paired_history) {
    const double dt =
        source_timestamp_seconds - left_state_candidate.source_timestamp_seconds;
    const Vec6 raw_left =
        poseErrorWorld(conditioned_left, manual_candidate.left) / dt;
    const Vec6 raw_right =
        poseErrorWorld(conditioned_right, manual_candidate.right) / dt;
  ...
  manual_candidate.left = conditioned_left;
  manual_candidate.right = conditioned_right;
  ...
  if (config_.dexhand_velocity_qp.pre_ik_conditioning.enabled) {
    left_pre_ik_conditioner_ = std::move(left_conditioner_candidate);
    right_pre_ik_conditioner_ = std::move(right_conditioner_candidate);
  }
  return true;
}

Pose TargetManager::conditionManualTarget(ArmSide side,
                                           const Pose& requested) {
  if (!config_.dexhand_velocity_qp.pre_ik_conditioning.enabled) {
    return requested;
  }
  return side == ArmSide::kLeft ? left_pre_ik_conditioner_.condition(requested)
                                : right_pre_ik_conditioner_.condition(requested);
}
